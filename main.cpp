...
void heap_insert_tests(max_heap &hp) {
	...
	// insert without any swap_ups needed
		// <INSERT TEST(S) HERE>
	std::cout << std::endl;	
	std::cout << "*** TESTING INSERT WITHOUT ANY SWAP_UPS NEEDED ***" << std::endl;
	max_heap hp1(10);
	std::cout << "adding item0,  with number 3 to heap1" << std::endl;
	hp1.insert(text_item{"item0", 3});
	temp = hp1.top();
	std::cout << "Top of heap is: " << temp << std::endl;
	std::cout << "adding item1,  with number 2 to heap1" << std::endl;
	hp1.insert(text_item{"item1", 2});
	temp = hp1.top();
	std::cout << "Top of heap is: " << temp << std::endl;
	std::cout << "adding item2,  with number 1 to heap1" << std::endl;
	hp1.insert(text_item{"item2", 1});
	temp = hp1.top();
	std::cout << "Top of heap is: " << temp << std::endl;
	std::cout << "adding item3,  with number 0 to heap1" << std::endl;
	hp1.insert(text_item{"item3", 0});
	temp = hp1.top();
	std::cout << "Top of heap is: " << temp << std::endl;
	...
	// insert with a swap_up / multiple swap_ups
		// <INSERT TEST(S) HERE>
	std::cout << std::endl;	
	std::cout << "*** TESTING INSERT WITH MULTIPLE SWAP_UPS ***" << std::endl;
	max_heap hp2(10);
	std::cout << "adding item0,  with number 1 to heap2" << std::endl;
	hp2.insert(text_item{"item0", 1});
	temp = hp2.top();
	std::cout << "Top of heap is: " << temp << std::endl;
	std::cout << "adding item1,  with number 2 to heap2" << std::endl;
	hp2.insert(text_item{"item1", 2});
	temp = hp2.top();
	std::cout << "Top of heap is: " << temp << std::endl;
	std::cout << "adding item2,  with number 3 to heap2" << std::endl;
	hp2.insert(text_item{"item2", 3});
	temp = hp2.top();
	std::cout << "Top of heap is: " << temp << std::endl;
}
...
void heap_delete_tests(max_heap &hp) {
	...
	// remove_max works when swap_down with left child
		// <INSERT TEST(S) HERE>
	std::cout << std::endl;	
	std::cout << "*** TESTING DELETEMAX (SWAP_DOWN WITH LEFT CHILD) ***" << std::endl;
	max_heap hp1(10);
	hp1.insert(text_item{"item0", 3});
	hp1.insert(text_item{"item1", 2});
	hp1.insert(text_item{"item2", 1});
	hp1.insert(text_item{"item3", 0});
	temp = hp1.delete_max();
	std::cout << "Item returned from heap1 delete: "<< temp << std::endl;
	temp = hp1.top();
	std::cout << "Top of heap1 is now: " << temp << std::endl;	
	// remove_max workd when swap_down with right child
		// <INSERT TEST(S) HERE>
	std::cout << std::endl;	
	std::cout << "*** TESTING DELETEMAX (SWAP_DOWN WITH RIGHT CHILD) ***" << std::endl;
	max_heap hp2(10);
	hp2.insert(text_item{"item0", 3});
	hp2.insert(text_item{"item1", 1});
	hp2.insert(text_item{"item2", 2});
	hp2.insert(text_item{"item3", 0});
	temp = hp2.delete_max();
	std::cout << "Item returned from heap2 delete: "<< temp << std::endl;
	temp = hp2.top();
	std::cout << "Top of heap2 is now: " << temp << std::endl;	
	// remove_max on an empty heap (should throw exception similar to top())
		// <INSERT TEST(S) HERE>
	std::cout << std::endl;	
	std::cout << "*** TESTING DELETEMAX ON AN EMPTY HEAP ***" << std::endl;
	max_heap hp3(10);
	try {
		temp = hp3.top(); 
		std::cout << "Top of heap3 is: " << temp << std::endl;	
...
void tree_tester(string_bst const &tree) {
	...
	std::cout << "Found: "<< to_find1 << 
		" in the input file " << num_times1 
		<< " time(s)." << std::endl;
	to_find1 = "is";
	num_times1 = tree.word_frequency(to_find1);
	std::cout << "Found: "<< to_find1 << 
		" in the input file " << num_times1 
		<< " time(s)." << std::endl;
...
void overall_most_freq(max_heap hp) {
	...
	if (hp.size() > 4) {
		std::cout << hp.delete_max() << std::endl;
		std::cout << hp.delete_max() << std::endl;
		std::cout << hp.delete_max() << std::endl;
		std::cout << hp.delete_max() << std::endl;
		std::cout << hp.delete_max() << std::endl;
...
void text_analysis_tester(string_bst &tree) {
	...
	std::cout << std::endl;
	at_least_length(copy_to_heap(tree), 12); // change the 8 to test other string-lengths
	std::cout << std::endl;
	starts_with(copy_to_heap(tree), 'x'); // change the 'c' to test words that starts_with_letter
