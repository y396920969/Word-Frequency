...
int string_bst::word_frequency_helper(node_t * n, tree_key const & key) const {
	if(n == NULL)
		return 0;
  	else if (n->data.word == key)
	  	return n->data.freq;
  	else if (key < n->data.word)
		return word_frequency_helper(n->left, key);
  	else return word_frequency_helper(n->right, key);
}
